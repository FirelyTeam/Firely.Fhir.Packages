...
public class CommonFhirPackageSourceTests
{
    ...
    private readonly FhirPackageSource _clientResolver = new(new ModelInspector(FhirRelease.STU3), PACKAGESERVER,
        [US_CORE_PACKAGE]);
    ...
    [TestMethod]
    public async Task Test400To401Redirect()
    {
        var source = new FhirPackageSource(new ModelInspector(FhirRelease.R4),
            "http://packages.simplifier.net", ["hl7.fhir.r4.core@4.0.0"]);

        var patientText = await source.ResolveByCanonicalUriAsyncAsString("http://hl7.org/fhir/StructureDefinition/Patient");
        patientText.Should().Contain("\"fhirVersion\":\"4.0.1\"");
    }
}